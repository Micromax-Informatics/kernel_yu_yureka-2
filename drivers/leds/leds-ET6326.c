...
int aw2013_set_color_singlecolor(struct aw2013_dev_data *aw2013)
{
	...
	if(aw2013->leds[LED_RED].blinking ||aw2013->leds[LED_GREEN].blinking || aw2013->leds[LED_BLUE].blinking)
	{
        ...
        if (aw2013->leds[LED_BLUE].blinking)
			...
			aw2013_i2c_write(0x0,0);
	if(red_on || green_on || blue_on)
	{
		if(0 == aw2013_has_inited){
			aw2013_i2c_write(0x0,0x18);//reset all register
			mdelay(1);
			aw2013_has_inited = 1; 
		}
		led_sutdown=0;
	printk("red:%d, green:%d, blue:%d,tp_color:%x\n", red_on, green_on, blue_on, tp_color);
        switch (tp_color) {
        case 0x31:   //white
        if(green_on)
		{
		aw2013_i2c_write(0x6, 0x0f);
		aw2013_i2c_write(0x8, 0x0f);
		}
		if(red_on)aw2013_i2c_write(0x7, 0x0f);//red is reg7
        break;
        case 0x34:  //yellow
        if(green_on)
		{
    ...
    if(blink_flag){
    		aw2013_i2c_write(0x1, 0x2a); //period 5.5s
			aw2013_i2c_write(0x2, 0x1e);
			aw2013_i2c_write(0x5, 0x11); //0.128s
			aw2013_i2c_write(0x4, (green_on&blink_flaggreen)<<5|(red_on&blink_flagred)<<3|(green_on&blink_flaggreen)<<1);
	}else {		
			aw2013_i2c_write(0x4, green_on<<4|red_on<<2|green_on);	// LED level,
	} 
	}
	else{	
...
{	
	...
	s_aw2013->leds[LED_RED].blinking = 0;
	s_aw2013->leds[LED_GREEN].blinking = 0;
	s_aw2013->leds[LED_BLUE].blinking = 0;

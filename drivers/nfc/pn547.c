...
static struct device_attribute pn547_dev_attr =
	__ATTR(nfc_debug, 0664 , nfc_show, nfc_store);
#endif
...
static int nfc_parse_dt(struct device *dev, struct pn547_dev *pdata)
{
	...
	//pdata->pwr_en = of_get_named_gpio(np, "nxp,pwr-en", 0);
	//if ((!gpio_is_valid(pdata->firm_gpio)))
	//		return -EINVAL;
...
//return 0;
	...
	//if(gpio_request(pn547_dev->pwr_en,"nxp,pwr-en") != 0)
	//{
	//  printk("PN547: gpio_firm_request error\n");
	//  goto err_firm;
	//}
...
//	gpio_direction_output(pn547_dev->pwr_en, 1);
//	gpio_direction_output(pn547_dev->pwr_en, 0);//Doesnot use enable pin, so pull down for current
	
	
//	gpio_set_value(pn547_dev->pwr_en, 1);
